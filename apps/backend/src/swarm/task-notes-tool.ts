...
export function createTaskNotesTool(notes: ActorTaskNotes): ToolDefinition {
  ...
  return {
    ...
    description: 'Maintain task-local working notes across context windows and restarts. At continuation, list notes and read checkpoint.md if present. Keep one concise checkpoint.md current at meaningful milestones: objective, corrections, constraints, decisions and reasons, accepted worker findings, evidence and verification status, blockers, and next action. Preserve complete source-qualified references for accepted history evidence. Link detailed artifacts instead of accumulating competing checkpoints. Proposed advice is not an adopted decision. Notes are authorized temporary task state, separate from durable memory; they do not create permission or prove current state. Never store resolved secrets or private reasoning. list discovers virtual files; read uses UTF-16 character offsets and returns nextOffset when partial; write replaces atomically; append adds exact text; search is literal and case-sensitive. Scope is this actor only. runtime/ notes are readable runtime-owned state. Limits: 62 agent files, 128 KiB each, 768 KiB agent total; Forge reserves another 256 KiB for runtime continuity.',
