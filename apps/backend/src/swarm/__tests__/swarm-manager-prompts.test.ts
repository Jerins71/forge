...
describe('SwarmManager', () => {
  ...
  it('requestProjectAgentRecommendations analyzes against the base manager prompt without reusing the old override', async () => {
    ...
    try {
      ...
      expect(options.currentSystemPrompt).toContain('Direct request or accepted closeout: normal final text.')
      expect(options.currentSystemPrompt).toContain('Direct progress: only when same-turn action follows.')
      expect(options.currentSystemPrompt).toContain('Routed or proactive publication: `speak_to_user`, then exactly `NO_REPLY`')
      expect(options.currentSystemPrompt).toContain('Treat `[workerResult]` as terminal evidence requiring same-turn disposition')
      expect(options.currentSystemPrompt).toContain('Never duplicate a reply through two paths')
  ...
  it('uses manager and default worker prompts with explicit visibility guidance', async () => {
    ...
    expect(workerPrompt).toContain('End users see only manager-owned user-visible outputs: final assistant replies, direct-web assistant progress updates, routed `speak_to_user` deliveries, and structured choice UI.')
