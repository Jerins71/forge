...
- **User-added OpenRouter models:** These start with manager access off. A model needs live-verified tool support, an explicit per-model **Manager agents** opt-in, and a configured OpenRouter key before an exact manager selection can use it. Each added card also has a **Context window** token input with **Apply** and **Reset**: a positive whole-number cap cannot raise capacity above the advertised window, blank Apply or Reset removes only the cap, and manager-enabled plus **Configure routing** stay independent. Caps persist under `openrouter:<model-id>` in `shared/config/model-overrides.json` and apply when an agent next starts. **Settings > Models** can also set shared OpenRouter routing/privacy defaults and exact-model routing overrides; those filters constrain OpenRouter inference routing only and are separate from specialist fallback.
