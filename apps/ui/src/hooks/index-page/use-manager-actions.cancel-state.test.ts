...
it('sends the selected Secure Sessions setting for cloned and local projects and resets the next project to off', async () => {
  client.createRepositoryProject = vi.fn(() => ({ requestId: 'clone-policy', promise: Promise.resolve({ manager: createdManager('clone'), repositoryPath: '/tmp/project' }), cancel: vi.fn() }))
  client.createManager = vi.fn(async () => createdManager('local'))
  client.validateDirectory = vi.fn(async () => ({ valid: true, path: '/tmp/project', message: null }))
  await renderHarness()
  await prepareClone()
  expect(latest!.newManagerSecureSessionsEnabled).toBe(false)
  await act(async () => latest!.handleNewManagerSecureSessionsEnabledChange(true))
  await act(async () => latest!.handleCreateManager(submitEvent()))
  expect(client.createRepositoryProject).toHaveBeenCalledWith(expect.objectContaining({ secureSessionsEnabled: true }), expect.anything())
  await act(async () => latest!.handleOpenCreateManagerDialog())
  expect(latest!.newManagerSecureSessionsEnabled).toBe(false)
  await act(async () => {
    latest!.handleNewManagerNameChange('Local')
    latest!.handleNewManagerCwdChange('/tmp/project')
    latest!.handleNewManagerModelSelectionChange({ provider: 'openai', modelId: 'gpt-5.5' })
  })
  await act(async () => latest!.handleCreateManager(submitEvent()))
  expect(client.createManager).toHaveBeenCalledWith(expect.objectContaining({ secureSessionsEnabled: false }))
})
