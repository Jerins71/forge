...
describe('browser workspace role-scoped IPC', () => {
  it('allows bounded projection commands only from the current popout and replies through main authority', async () => {
    ...
    await handlers.get(BROWSER_WORKSPACE_IPC.publish)!(eventFor(main), projection)
    ...
    expect(() => handlers.get(BROWSER_WORKSPACE_IPC.publish)!(eventFor(popout), projection)).toThrow('restricted to the main Forge renderer')
    ...
    const result = handlers.get(BROWSER_WORKSPACE_IPC.command)!(eventFor(popout), request)
    expect(main.webContents.send).toHaveBeenCalledWith(BROWSER_WORKSPACE_IPC.commandForward, request)
    await handlers.get(BROWSER_WORKSPACE_IPC.commandReply)!(eventFor(main), { requestId: 'request-123', ok: true, value: 'accepted' })
    await expect(result).resolves.toBe('accepted')
    await expect(handlers.get(BROWSER_WORKSPACE_IPC.command)!({ sender: { id: 99 }, senderFrame: null }, request)).rejects.toThrow('restricted to the current Managed Browser pop-out')
    installed.dispose()
  })
  ...
  it('rejects remote/null and stale workspace identities before forwarding', async () => {
    ...
    await (handlers.get(BROWSER_WORKSPACE_IPC.publish) as never as Function)(eventFor(main), { ...projection, workspaceEpoch: 8, sessionAgentId: null, profileId: null, snapshot: null })
    await expect((handlers.get(BROWSER_WORKSPACE_IPC.command) as never as Function)(eventFor(popout), { requestId: 'request-456', workspaceEpoch: 7, sessionAgentId: 'remote', profileId: 'remote', deadlineAt: new Date(Date.now() + 1000).toISOString(), command: { type: 'open' } })).rejects.toMatchObject({ code: 'unavailable-host' })
    expect(main.webContents.send).not.toHaveBeenCalledWith(BROWSER_WORKSPACE_IPC.commandForward, expect.anything())
  })

  it('publishes full-affinity preview scope only to main and strips it from pop-out delivery', async () => {
    const handlers = new Map<string, (...args: never[]) => unknown>()
    const main = windowWithId(10); const popout = windowWithId(20)
    const publishPreviewScope = vi.fn()
    const installed = installBrowserWorkspaceIpc({
      ...
      getPopoutWindow: () => popout as never,
      viewHost: {} as never,
      getMode: () => 'docked',
      popOut: vi.fn(),
      dock: vi.fn(),
    ...
    const previewScope = {
      ...
      sessionRevision: 1,
      connected: true,
      tabs: [{ tabId: 'ext.profile.7', targetAffinity: 'external-chrome' as const, lifecycle: 'ready' as const, label: null, presented: false }],
    }
    await (handlers.get(BROWSER_WORKSPACE_IPC.publish) as never as Function)(eventFor(main), { ...projection, previewScope })
    expect(publishPreviewScope).toHaveBeenCalledWith({
      workspaceEpoch: 7,
      sessionAgentId: 'local-session',
      profileId: 'local-profile',
      scope: previewScope,
    })
    expect(popout.webContents.send).toHaveBeenLastCalledWith(
      BROWSER_WORKSPACE_IPC.projection,
      expect.not.objectContaining({ previewScope: expect.anything() }),
    )
    expect((handlers.get(BROWSER_WORKSPACE_IPC.snapshot) as never as Function)(eventFor(popout)))
      .not.toHaveProperty('previewScope')

    expect(() => (handlers.get(BROWSER_WORKSPACE_IPC.publish) as never as Function)({
      sender: main.webContents,
      senderFrame: { detached: false },
    }, { ...projection, previewScope })).toThrow('restricted to the main Forge renderer')
    expect(() => (handlers.get(BROWSER_WORKSPACE_IPC.publish) as never as Function)(eventFor(main), {
      ...projection,
      previewScope: { ...previewScope, tabs: [{ ...previewScope.tabs[0], label: 'leaked title' }] },
    })).toThrow('Browser preview tab membership is invalid')
    installed.dispose()
    expect(publishPreviewScope).toHaveBeenLastCalledWith(null)
  })
