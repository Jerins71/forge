...
void app.whenReady().then(async () => {
  ...
  (async()=>{try{
    const secureControlAvailable=typeof window.electronBridge.secureControlToken==='string'&&window.electronBridge.secureControlToken.length>=32;
    const bridge=window.electronBridge.browserAutomation;
    const now=new Date().toISOString();const tab={targetAffinity:'managed-electron',tabId:'fixture-tab',sessionAgentId:'fixture-session',profileId:'fixture-profile',url:${JSON.stringify(fixtureUrl)},title:'Forge Browser Fixture',lifecycle:'ready',loading:false,live:false,canGoBack:false,canGoForward:false,zoomFactor:1,controller:'none',agentCursor:null,recording:null,viewportSetting:{mode:'fill'},renderedViewport:null,error:null,createdAt:now,updatedAt:now};
    const session={schemaVersion:2,sessionAgentId:'fixture-session',profileId:'fixture-profile',hostingState:'hosted',tabs:[tab],activeTabId:'fixture-tab',defaultTabId:'fixture-tab',panelVisible:true,recentActions:[],revision:1,createdAt:now,updatedAt:now};
    ...
    const presentation=await bridge.setTabPresentation({tabId:'fixture-tab',visible:true,renderedViewport:{width:800,height:600,deviceScaleFactor:window.devicePixelRatio||1},hostGeneration:1,sessionRevision:1,sequence:1,workspaceEpoch:1});
    ...
    await bridge.setTabPresentation({tabId:'fixture-tab',visible:false,renderedViewport:null,hostGeneration:1,sessionRevision:2,sequence:2,workspaceEpoch:1});const typedFailure=await call('recordingStart',{});await bridge.setTabPresentation({tabId:'fixture-tab',visible:true,renderedViewport:{width:800,height:600,deviceScaleFactor:window.devicePixelRatio||1},hostGeneration:1,sessionRevision:3,sequence:3,workspaceEpoch:1});
    ...
    const passed=secureControlAvailable&&failures.length===0&&presentation.applied===true&&status?.result?.physicalTabVisible===true&&status?.result?.panelVisible===true&&evaluated?.result?.value?.clicks===1&&evaluated?.result?.value?.keys===1&&evaluated?.result?.value?.keyDowns>=2&&evaluated?.result?.value?.keyUps>=2&&evaluated?.result?.value?.typed==='fixture typeda'&&printable.ok&&snapshot?.result?.screenshot?.data?.length>0&&typedFailure?.error?.code==='recording-requires-visible-tab'&&typedFailure?.error?.retryable===true&&interrupted?.error?.code==='control-interrupted'&&stopped?.result?.sizeBytes>0&&close.tabUnavailable;
    console.log('FORGE_BROWSER_FIXTURE_PAGE='+JSON.stringify({passed,secureControlAvailable,operations:responses.map(response=>response.operation),failures,visibility:{presentation,status:status?.result},typedFailure:typedFailure.ok?null:typedFailure.error,interruption:interrupted.ok?null:interrupted.error,keyboard:evaluated?.result?.value,recording:stopped?.ok?{mimeType:stopped.result.mimeType,sizeBytes:stopped.result.sizeBytes,path:stopped.result.path}:null,close}));
