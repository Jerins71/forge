...
- **Main process** (`src/main.ts`) — launches the packaged backend, owns application windows and updates, composes one protocol-v2 `AutomaticBrowserHost`, and owns the RAM-only read-only Browser Preview deck
- **Trusted preload** (`src/preload.ts`) — constructs role-scoped bridges. The main renderer gets bounded browser/workspace/Chrome setup APIs; the pop-out gets only its workspace bridge; the sandboxed Preview renderer gets only preview snapshot, frame-pull, and shell-command IPC
- **Renderer process** — loads the staged UI bundle from `ui/index.html`. The main role registers one Desktop browser host with the local Builder backend, projects canonical browser-tab state, and renders either the embedded workspace or a Chrome-backed tab card. The renderer transiently relays complete `browser_status` inventory responses between the trusted bridge and backend, but does not project that inventory into Browser workspace UI or canonical renderer state; it also receives no per-tab operation-authority state or controls. The Preview role mounts only the read-only image deck and opens no backend socket
...
`BrowserPreviewHost` observes only explicit cards in the selected local workspace. A presented embedded card receives bounded `capturePage(undefined, { stayHidden: true })` thumbnails without changing native view ownership, focus, visibility, or source viewport. A Chrome card mirrors only an already-successful exact-target agent snapshot; preview state never lists, acquires, executes, renews, releases, or polls Chrome authority. One latest image per card is delivered through renderer pull IPC, discarded after privacy/lifecycle invalidation, and never enters browser persistence or backend transport. The [Browser automation guide](../../docs/BROWSER_AUTOMATION.md) is the user-facing source of truth.
...
| `src/browser/browser-workspace-ipc.ts` | Narrow role-scoped pop-out projection, atomic preview membership publication, and correlated command relay |
| `src/browser/browser-preview-host.ts` | RAM-only card membership, bounded capture scheduling, passive Chrome observation, privacy state, and frame-pull ownership |
| `src/browser/browser-preview-ipc.ts` | Exact-window/main-frame validation for preview open, frame pull, state, and shell commands |
