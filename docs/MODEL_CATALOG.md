...
The dialog groups **Privacy** and **Providers** first. **Advanced routing** and **Scope & privacy details** start collapsed. The footer is **Save changes** / **Cancel**. Each field has a source badge (**Default**, **Override** or **Custom**, **Cleared**, **Required**) whose menu is **Use shared default** on a model override, **Use OpenRouter default** on shared defaults, and **Clear · no additional restriction**.

Field semantics are product-facing, not a merge API: an absent field inherits the shared default (or OpenRouter's unrestricted default when Forge has none); `null` clears the field; arrays (`order`, `only`, `ignore`, `quantizations`) and `max_price` replace rather than merge. Shared **Zero data retention** and **Block data collection** are a privacy floor that a per-model override cannot weaken, including with a previously saved clear.

The controls are:

- **Privacy** — **Zero data retention** and **Block data collection** are separate filters. Required ZDR limits routing to eligible zero-retention endpoints and still permits implicit in-memory prompt caching. Blocking collection/training is not a ZDR guarantee. OpenRouter account guardrails still apply. See [ZDR](https://openrouter.ai/docs/guides/features/zdr), [provider routing](https://openrouter.ai/docs/guides/routing/provider-selection), and [OpenRouter privacy settings](https://openrouter.ai/settings/privacy).
- **Providers** — **Automatic**, **Prefer providers**, or **Only selected**. **Automatic** lets OpenRouter choose from eligible providers. **Prefer providers** (`order`) is ordered preference, not a hard allowlist; other eligible providers may be used. **Only selected** (`only`) is a hard allowlist. Under **Only selected → Preference within selection**, that allowlist can also carry an ordered `order` preference. **Provider fallback** (`allow_fallbacks`) is OpenRouter endpoint fallback among still-eligible providers. It is distinct from Forge automatic model fallback across models or providers.
- **Advanced routing** — **Optimize for** (`sort`) selects lowest price, highest throughput, or lowest latency and is mutually exclusive with preferred order. **Price limits** (`max_price`) are optional USD-per-million input/output hard filters, not a session budget; a custom price object replaces the inherited one. **Require parameter support** filters to endpoints that support all request parameters; it is not auto-enabled, is recommended for tool-using agents, and may reduce availability. **Allowed quantizations** is an optional hard filter on endpoint formats (`int4`, `int8`, `fp4`, `fp6`, `fp8`, `fp16`, `bf16`, `fp32`, `unknown`). **Excluded providers** (`ignore`) cannot serve the request and must not conflict with the allowlist.
