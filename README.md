...
**Context doesn't die.** When Claude Code compacts, you get amnesia. Forge's default Summary compaction writes structured handoff files, retains the most recent context, and summarizes the rest. Experimental Context v2 can instead continue from a checkpoint and retrieve older history with the agent-only `history` tool. Compact and Smart compact follow the session's effective policy. On Summary, manual Smart compact on an already-idle Pi-backed manager stays idle, while active or interrupted work resumes. Context v2 rejects busy manual attempts until idle, and idle stays idle. Active compaction or context recovery also shows a violet pulsing `C` badge in the sidebar. Pin critical messages and they'll survive every compaction. The pin navigator in the chat header lets you jump directly to any pinned message. Conversations that have compacted 50+ times still maintain coherence.
...
**Desktop release safety:** `pnpm package:electron` is the build step. It now treats `apps/electron/release/` as ephemeral output for the current run, so stale assets are cleared before packaging. Official desktop releases still follow a build-first, publish-last, draft-first flow. Desktop rollout is beta-first: beta versions must be published as GitHub prereleases, and stable release happens later as a separate intentional promotion. The old `pnpm release:electron` shortcut is intentionally disabled. The project-scoped release skill lives at `~/.forge/profiles/middleman-project/pi/skills/electron-release/`. Official cuts are permitted only from a selected clean synchronized `main` or `release/vMAJOR.MINOR.PATCH` branch. Its `release.sh --version VERSION` stage updates **both** `version.json` and `apps/electron/package.json`, commits and pushes those two authorities to the selected branch, then automatically exits before macOS packaging, Windows packaging or CI `workflow_dispatch`, draft creation, tagging, asset upload, or publishing. Run one complete exact-SHA candidate preflight against that new final release commit. Only after that preflight passes, invoke the project-scoped script again without `--version`, with `--expected-sha` and `--evidence`, for packaging. See the workflow documented in [`apps/electron/README.md`](apps/electron/README.md): dispatch and register the exact-SHA Windows `workflow_dispatch` run first, then build macOS locally, collect the already-registered Windows artifacts after macOS cleanup, create a draft GitHub Release, keep beta builds marked as prereleases, then upload the full updater asset set (installers, archives, `latest*.yml`, `*.blockmap`, and related release files) before publishing.
...
Session forking lets you run discovery in one conversation, gather context, narrow down an approach, then fork into parallel workstreams that all inherit that context. You can fork from the current point or from any earlier message, carrying forward only the relevant context. It's branching for conversations. Cursor SDK runtime state and usage records are omitted from forks so resumed branches do not leak SDK state or double-count usage. Initial Pi model-input captures, Pi response-throughput measurements, and historical Codex sidecar display cards are also omitted, so a Pi-backed fork records its own initial request and begins its own response-throughput history.
...
On **Summary**, when a session reaches ~85% context capacity, Forge pauses and writes a structured markdown handoff file capturing the current operational state, then compacts. The compacted context retains the most recent ~20,000 tokens verbatim and summarizes everything older. The handoff file ensures no critical context is lost. **Context v2** is experimental and opt-in: it continues at native settled threshold or overflow boundaries rather than that ~85% trigger. Compact and Smart compact use a deterministic checkpoint instead of an LLM summary, skip the Smart handoff, and leave older history recoverable through the agent-only `history` tool (`sessions`, `search`, `read`). There is no human history drawer. Context v2 is executable only on supported ordinary Pi Builder managers (OpenAI/Codex or Anthropic). Collaboration, Cortex/system, Cursor SDK, plugin/external threads, and workers cannot run it; workers inherit the owning manager.

While compaction or context recovery is active, the session row shows a violet pulsing `C` badge in the sidebar. Compact and Smart compact follow the saved policy at the next context transition. On Summary, if you trigger Smart compact manually on an already-idle Pi-backed manager, it stays idle afterward; if the manager is active, interrupted, or dispatch-pending, it resumes after compaction. On Context v2, a busy manual attempt is rejected until idle, and an idle manager stays idle. **Settings → General → Compaction** still controls the compaction model, reasoning, and timeout for supported Pi-backed OpenAI/Codex and Anthropic manager compaction runtimes only, not Cursor SDK, xAI/Grok, or user-added OpenRouter manager models; OpenRouter manager eligibility is a separate policy and OpenRouter models are not compaction choices. Context continuation (Summary vs Context v2) lives in **Project Settings → Context management**, with an optional session override beside Send.

You can pin up to 10 messages per session (user or assistant) by clicking the pin icon. Pinned messages are preserved through compaction — under Summary their full content is injected into the summary under a dedicated section, and under Context v2 they are included in the checkpoint.
...
- **Unified local/remote sidebar** — the new project view is on by default (**Settings → General → Sidebar → Use new project view**), with Classic as the rollback. It opens in **Inbox**, with **Projects** keeping the familiar project/session tree. Inbox lists **Needs you**, **Active**, and **Recent**, then an inline Projects tree. **Active** is capped at five currently working sessions, with **N more** opening Projects. **Recent** lists idle sessions updated in the last 7 days, excluding **Needs you** and **Active**; it initially shows five rows, **Show N more** expands the remaining matching rows inline in Inbox, and **Show less** collapses back to five without switching to Projects. **Needs you** is server-issued work-lifecycle attention. Unread and error badges do not create it, and the UI does not synthesize it from local badges. An unanswered **Input requested** pending choice raises **Needs you** even while work still runs or follow-ups are queued; answering or cancelling retracts that item without marking the session complete, so remaining work can stay **Active**. **Done** and **Clear** dismiss those exact items; mute only hides **Needs you** and the Inbox badge. Origins that do not advertise session attention omit **Needs you**. Project headers still expand or collapse nested sessions and do not select a conversation. Enabled remote headers appear blue with a globe marker beside local projects, while their session rows use status dots. Remote project actions are limited: **Change Working Directory** is available on the project header through the server directory browser, while local rename, archive, delete, fork, and model actions are absent. Connections can show connecting, sign-in-required, unreachable, server-disabled, update-required, or empty states. Selecting a remote session makes its server the active origin for supported project surfaces; selecting a local session switches back. Author chips identify other users in remote chat, while the viewer indicator is presence for subscribed session viewers only—not typing, editing, or locking. The saved mixed order is a local preference and does not grant access to any remote project. On desktop, Cortex lives in the activity-rail popover; mobile and Classic keep the pinned sidebar row.
- **Project Settings** — for a local Builder project, right-click its header or use its hover/focus **…** menu, then choose **Project Settings**. The page remains scoped to that selected project and groups project name, working directory, default model and reasoning, **Context management** (Summary default or experimental Context v2), Project secrets, and **Project Settings → Repository resources** controls for the repo-root `.forge` inventory, executable trust, and directory override. Existing Rename, Change Default Model, Change Working Directory, and Project Secrets context-menu shortcuts remain. Cortex and Remote Projects do not offer Project Settings.
- **Workspace rail** — the desktop app adds a left activity rail for Chat, Browser, Files, Source Control, Terminal, Cron/Schedules, and Artifacts/Dashboard. Chat returns to the current manager/session conversation, including from a selected worker route back to its parent manager thread. Browser opens one automatic local workspace for the current manager's embedded Forge tabs; transient Chrome-backed targets remain entirely in Chrome and do not appear in its tab strip. Files opens in a left split pane beside the rail with a resizable tree and file surface for direct text editing or previews. Source Control opens inline with Changes and History stacked in one explorer, plus Worktrees and Pull Requests shortcuts. Selecting a worktree changes the Source Control and Files browsing context without changing the chat session CWD. Browser, Files, Source Control, Artifacts/Dashboard, and Schedules switch mutually exclusively so panes do not stack or hide behind each other. Terminal stays independent and persistent instead of being part of that surface switch. Mobile keeps the header/drawer workspace controls.
- **Browser automation** — Forge Desktop's local Automatic Browser can render an embedded tab in Forge or control a Chrome-backed target that stays entirely in Chrome without adding it to the Browser workspace. An enabled and authenticated Forge extension grants profile-wide access to eligible ordinary web tabs; `browser_status` returns a bounded `eligibleTabs` inventory across ready authenticated profiles; its transient manager/model metadata is not shown in Browser workspace UI or persisted renderer state. Tabless `browser_open(reuseExistingTab: true)` selects the active/most-recent eligible tab without OS focus. Passing an inventory `tabId` selects that exact tab; non-open operations remain sticky. Trusted clicks, key presses, scrolls, and touches interrupt agent work; Forge settles any in-flight command, preserves the exact tab lease when safe, and requires a fresh snapshot before another mutation. Ordinary idle pointer movement does not interrupt, but a mismatched synthetic-sequence pointer move does. Eligible navigation re-proves the same root and remains on the same Chrome target; explicit targets never migrate. **Take Control** releases that exact authority for terminal human takeover. `reuseExistingTab: false`, or no eligible tab, may create an inactive neutral `about:blank` tab for one authorized initial navigation. There is no profile confirmation prompt or picker, and Chrome-internal/restricted pages remain excluded by platform capability. Embedded tabs include viewport, transient screenshot, recording, and dock/pop-out controls; Chrome omits embedded-only controls. Set up optional Chrome use under **Settings → Use Chrome with Forge**. The capability is not forwarded to Remote Projects or Collaboration. See [Browser automation](docs/BROWSER_AUTOMATION.md) for behavior, setup, persistence, and security boundaries.
...
- **Source Control** — review current changes, commit history, worktrees, and GitHub pull requests from the workspace rail. Changes and History are stacked in one explorer you can collapse or resize; History shows commit refs such as `main` and `origin/main` plus a compact connected graph for workspace repositories, so unpublished local commits sit above the last pushed remote tip. Forge may quietly auto-fetch stale origin data when you enter Source Control or change context, while manual Fetch stays explicit and reports errors. Branch actions support fetch, switch branch, create branch, fast-forward-only pull, **Publish Branch** for a branch with no upstream, and **Sync Changes** for later updates. **Publish Branch** fetches origin and publishes without force; Forge refuses to overwrite when the same-named `origin/<branch>` has commits missing locally. If the remote branch is created but local upstream tracking cannot be set, publishing still succeeds, `origin/<branch>` remains the destination, and Forge shows a tracking warning. **Sync Changes** fetches origin and then pushes the current branch to its upstream when it is ahead and not behind. Confirmation and preflight checks run before any write. File create, rename, save, and delete operations refresh Changes. Opening Source Control preserves Files drafts; branch switch/create and fast-forward-only pull guard dirty tabs in the matching worktree with Save, Discard, or Cancel. Pull Requests uses the `gh` CLI and shows unavailable or degraded states when there is no GitHub remote, `gh` is missing, or `gh` is not authenticated. PR merge requires confirmation, re-checks the head commit with GitHub's match-head-commit guard, and does not delete branches or use admin bypass. Force push, stash, discard, rebase, branch deletion, and worktree create/remove are intentionally out of scope.
...
- **Working plans** — substantial work can expose a compact current checklist in the chat header and transcript. Work graphs add **Graph** and **List** views: select a step to inspect assignment, acceptance criteria, authoritative status, unresolved dependencies, and execution attempts, and that selection survives switching views. Worker success stays **awaiting review** until manager acceptance.
...
- **Stats** — usage and activity summaries, Token Analytics, and a **Response throughput** tab. Response throughput records manager and worker model calls made through the Pi runtime. Its final `tok/s` rate is provider-final output tokens divided by the complete monotonic request-start-to-terminal `requestWallMs` duration (`request_wall_monotonic`); TTFT and output-tail timing are diagnostics, not rate denominators. The browser-local, default-off **Settings → General → Show response throughput in conversations** preference controls only the manager and worker conversation indicators; when enabled, they show only provider-final `tok/s` (or `—`), never an estimate. It does not affect measurement, durable records, or **Stats → Response throughput** history. The regenerable v2 cache rebuilds from durable records instead of reusing first-output-tail rates. Response throughput is separate from overview and Token Analytics usage totals. Session totals are historical and include archived data, while active counts exclude archived projects and sessions. When Cursor SDK is used, its usage is included in dashboard stats, token analytics, and telemetry provider inference.
